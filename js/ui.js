...
(function () {
  ...
  function addBanner(outcome) {
    ...
    var title = outcome === 'won' ? '通關' : (outcome === 'secret_won' ? '隱藏結局' : '失敗');
  ...
  function cardAlt(card) {
    var lines = [card.title, card.endingName, card.characterName + '　' + card.className];
  ...
  function addEndingCard() {
    ...
    fig.appendChild(img);
    fig.appendChild(el('figcaption', 'ending-cap', card.characterName + '　·　' + card.className + '　·　' + card.endingName));
  ...
  function renderActions(st) {
    ...
    if (st.status === 'won' || st.status === 'secret_won' || st.status === 'lost') {
      ...
      row.appendChild(button('從頭再玩一次', '同一個角色，全新狀態，回到起點', null, function () {
        act({ type: 'restart' });
      }));
